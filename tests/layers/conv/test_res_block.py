...
from dl_toolkit.modules.layers.conv.resnet.res_block import ResidualBlock
...
def test_forward_output_is_sum():
    ...
    expected = block.last_act(expected)  # Account for last_act
...
def test_zero_init_residual_separable(norm_layer):
    ...
    if norm_layer == "batch":
        ...
        conv = block.conv2.conv
        assert isinstance(conv, SeparableConv2d), "Expected SeparableConv2d"
        pointwise_conv = conv.module[-1]
        assert torch.all(pointwise_conv.weight == 0)
        if pointwise_conv.bias is not None:
            assert torch.all(pointwise_conv.bias == 0)
