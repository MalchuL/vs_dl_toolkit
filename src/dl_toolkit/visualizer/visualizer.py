...
class Visualizer:
    """Visualizer class that uses the ``Matplotlib`` library as the backend. It has the following
    functions:
    ...
    Examples:
        ...
        >>> vis.draw_texts(text='Some text',
        >>>               position=np.array([2, 2]),
        >>>               colors='b')
        >>> vis.draw_texts(text=['Some text','Other text'],
    ...
    def _init_manager(self, win_name: str) -> None:
        ...
        if getattr(self, "manager", None) is None:
            self.manager = new_figure_manager(num=1, FigureClass=Figure, **self.fig_show_cfg)
    ...
    def draw_points(
        self,
        positions: Union[np.ndarray],
        colors: Union[str, tuple, List[str], List[tuple]] = "g",
        marker: Optional[str] = None,
        sizes: Optional[Union[np.ndarray]] = None,
    ) -> "Visualizer":
        ...
        Args:
            ...
            sizes (Optional[Union[np.ndarray, torch.Tensor]]): The marker size.
                Defaults to None.
        """
        check_type("positions", positions, (np.ndarray,))
    ...
    def draw_texts(
        ...
        font_properties: Optional[Union["FontProperties", List["FontProperties"]]] = None,
    ) -> "Visualizer":
        ...
        check_type_and_length("font_sizes", font_sizes, (int, float, list), num_text)
    ...
    def draw_lines(
        self,
        x_datas: Union[np.ndarray],
        y_datas: Union[np.ndarray],
        colors: Union[str, tuple, List[str], List[tuple]] = "g",
    ...
    def draw_circles(
        self,
        center: Union[np.ndarray],
        radius: Union[np.ndarray],
        ...
        Args:
            ...
            face_colors (Union[str, tuple, List[str], List[tuple]]):
                The face colors, colors to fill inside shape. Defaults to None.
        ...
        check_type("center", center, (np.ndarray,))
        check_type("radius", radius, (np.ndarray,))
        if len(center.shape) == 1:
            center = center[None]
        assert center.shape == (radius.shape[0], 2), (
            "The shape of `center` should be (radius.shape, 2), " f"but got {center.shape}"
        )
        if not (
    ...
    def draw_bboxes(
        ...
        Args:
            ...
            line_widths (Union[Union[int, float], List[Union[int, float]]]):
                ...
                have the same linewidth. Defaults to 2.
            face_colors (Union[str, tuple, List[str], List[tuple]]):
                The face colors, colors to fill inside shape
            alpha (Union[int, float]): The transparency of bboxes.
                Defaults to 0.8.
        """
        check_type("bboxes", bboxes, (np.ndarray,))
    ...
    def draw_polygons(
        self,
        polygons: Union[Union[np.ndarray], List[Union[np.ndarray]]],
        edge_colors: Union[str, tuple, List[str], List[tuple]] = "g",
        line_styles: Union[str, List[str]] = "-",
        line_widths: Union[Union[int, float], List[Union[int, float]]] = 2,
        face_colors: Union[str, tuple, List[str], List[tuple]] = "none",
        ...
        check_type("polygons", polygons, (list, np.ndarray))
        edge_colors = color_val_matplotlib(edge_colors)  # type: ignore
        face_colors = color_val_matplotlib(face_colors)  # type: ignore
    ...
    def draw_binary_masks(
        ...
        try:
            ...
            raise ImportError("Please install opencv-python to use draw_binary_masks function")
        check_type("binary_masks", binary_masks, (np.ndarray,))
        assert binary_masks.dtype == np.bool_, (
            "The dtype of binary_masks should be np.bool_, " f"but got {binary_masks.dtype}"
        )
        ...
        self.ax_save.imshow(img, extent=(0, self.width, self.height, 0), interpolation="nearest")
        return self
