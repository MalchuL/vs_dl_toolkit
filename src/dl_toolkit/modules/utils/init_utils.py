...
def init_weights(net, init_type='kaiming_uniform', gain: float | None = None,
                 nonlinearity='linear'):
    def init_func(m):
        ...
        if hasattr(m, 'weight') and (
            ...
            if init_type == 'normal':
                ...
                # TODO add weight clipping
                init.kaiming_normal_(m.weight.data, a=gain_value, mode='fan_in',
                                     nonlinearity=nonlinearity)
            elif init_type == 'kaiming_uniform':
                if gain_value is None:
                    gain_value = 0.0
                # gain converts to = math.sqrt(2.0 / (1 + gain ** 2))
                init.kaiming_uniform_(m.weight.data, a=gain_value, mode='fan_in',
                                      nonlinearity=nonlinearity)
            ...
            if hasattr(m, 'bias') and m.bias is not None:
                ...
                'GroupNorm') != -1) and hasattr(m, 'weight') and m.weight is not None:
